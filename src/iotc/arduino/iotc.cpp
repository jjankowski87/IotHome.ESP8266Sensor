...
unsigned long _getNow() {
  ...
  IPAddress addresses[ADDRESS_COUNT] = {
    IPAddress(129, 6, 15, 28),   // time.nist.gov
    IPAddress(129, 6, 15, 29),   // time.nist.gov
    IPAddress(130, 149, 17, 8),  // toto-link
    IPAddress(91, 212, 242, 20), // 0.pl.pool.ntp.org
    IPAddress(193, 219, 28, 2)   // 1.pl.pool.ntp.org
  };

retry_getNow:
  IPAddress address = addresses[retryCount % ADDRESS_COUNT];
  ...
  if (Udp.parsePacket()) {
    ...
    if (retryCount < RETRY_COUNT) {
      retryCount++;
      IOTC_LOG(F("ERROR: couldn't connect to NTP server, trying again."));
      goto retry_getNow;
    }
    IOTC_LOG(F("ERROR: couldn't fetch the time from NTP."));
  }
